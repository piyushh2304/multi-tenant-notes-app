...
export default function AppPage() {
  ...
  async function loadNotes() {
    ...
    try {
      ...
      setError(
        "Network error loading notes. Please check your connection or server.",
      );
    }
  }
  ...
  useEffect(() => {
    ...
    if (
      qp.get("checkout") === "success" &&
      localStorage.getItem("upgrade-intent") === "1"
    ) {
  ...
  async function upgrade() {
    ...
    try {
      ...
      if (res.ok) {
        const updated = {
          ...session,
          tenant: { ...session.tenant, plan: data.plan },
        };
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white border rounded-xl p-4">
            ...
            <textarea
              ...
              placeholder="Content"
              className="w-full mb-3 rounded-md border px-3 py-2 h-24"
            />
            <button
              onClick={createNote}
              disabled={isLimited}
            ...
            {isFree && isMember && (
              <p className="text-xs text-muted-foreground mt-2">
                Free plan limited to 3 notes per tenant for members.
              </p>
            )}
          </div>
          <div className="bg-white border rounded-xl p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-3xl mx-4 bg-white rounded-2xl border shadow-xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="border rounded-xl p-5">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Free plan with up to 3 notes for members.
                </p>
                <div className="text-3xl font-extrabold">$0</div>
                {stripeCfg?.paymentLinkBasic ? (
                  <a
                    href={stripeCfg.paymentLinkBasic}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-4 inline-block px-4 py-2 rounded-md border hover:bg-accent"
                  >
                    Purchase
                  </a>
                ) : (
                  <button
                    disabled
                    className="mt-4 px-4 py-2 rounded-md border opacity-60 cursor-not-allowed"
                  >
                    Current
                  </button>
                )}
              ...
              <div className="border rounded-xl p-5">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Unlimited notes for everyone in your tenant.
                </p>
                <div className="text-3xl font-extrabold">$5</div>
                {stripeCfg?.paymentLinkPro ? (
                  <a
                    href={stripeCfg.paymentLinkPro}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-4 inline-block px-4 py-2 rounded-md bg-primary text-primary-foreground hover:opacity-90"
                  >
                    Purchase
                  </a>
                ) : (
                  <button
                      ...
                      try {
                        ...
                        if (data.url) {
                          ...
                          if (session?.user?.role === "admin") {
                            ...
                            alert(
                              "Payment not available. Please ask your admin to upgrade.",
                            );
                          }
                        }
                      } catch (e) {
                    ...
                    className="mt-4 px-4 py-2 rounded-md bg-primary text-primary-foreground hover:opacity-90"
                  >
                    Purchase
                  </button>
                )}
            ...
            <p className="text-xs text-muted-foreground mt-4">
              Test mode. Use Stripe test cards when prompted.
            </p>
          </div>
        </div>
      )}
