      ...
      <main className="max-w-5xl mx-auto px-4">
        <section className="py-12 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            >
              Secure, isolated notes for each company. Admins manage users and
              subscriptions; members focus on notes.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Use any predefined account. Password is "password".
            </p>
            <label className="block text-sm font-medium mb-1">Tenant</label>
            <select
              value={tenantSlug}
              onChange={(e) => setTenantSlug(e.target.value)}
              className="w-full mb-3 rounded-md border px-3 py-2 bg-background"
            >
            ...
            <input
              ...
              className="w-full mb-4 rounded-md border px-3 py-2 bg-background"
            />
            {error && <div className="text-red-600 text-sm mb-3">{error}</div>}
            <button
              disabled={loading}
              className="w-full py-2 rounded-md bg-primary text-primary-foreground font-semibold hover:opacity-90 disabled:opacity-60"
            >
              {loading ? "Signing in..." : "Sign in"}
            </button>
            <p className="text-xs text-muted-foreground mt-3">
              No account? Sign up creates a member in the selected tenant.
            </p>
            <button
              type="button"
              onClick={async () => {
